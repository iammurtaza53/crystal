...
def greeting():
    save_confirmation = input(
        'Have you refreshed and saved all the books? (type "yes" if you have) >> ')
    if save_confirmation != 'yes':
        print('That was a wrong answer.')
        input('Hit Enter to exit')
        exit()
    else:
        save_confirmation2 = input(
            'ARE YOU SURE??? (type "I am" if you are sure) >> ')
        if save_confirmation2 != 'I am':
            input('Hit Enter to exit')
            exit()

    reconcile_confirmation = input(
        'Have you RECONCILED all the books? (type "YES" if you have) >> ')
    if reconcile_confirmation != 'YES':
        print('How can you trade if you don\'t even know what your positions are?')
        input('Hit Enter to exit')
        exit()
    else:
        reconcile_confirmation2 = input(
            'Look, you think you have reconciled, but what if you haven\'t? ARE YOU SURE? (type "Reconciled" if you are sure) >> ')
    ...
    while True:
        ...
        if len(confirm) == 0:
            ...
            if len(invalid_tickers(confirm)) == 0 or confirm == ['--ALL']:
                ...
                print('You\'ve entered some invalid tickers, bro: ' +
                      str(invalid_tickers(confirm)) + '. Try again.')
    return result
...
def show_done_message():
    ...
    print()
    print()
    print('  >>  Make sure to double-check your trades.')
    print()
    print()
    print()
    input('  >>  Hit Enter to exit')
