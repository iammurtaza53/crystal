...
class TradeGenerator(object):
    ...
    def fold_flips_to_new_rows(self):
        ...
        self.output = self.output.drop(['second_leg', 'amount_2nd_leg', 'amount_2nd_leg_LL_on'], axis = 1)
    ...
    def consolidate_order_rows(self, components):
        ...
        result.alloc_Jump = round(float(qty_by_strategy['jump'] / total_qty), 2)
        result.alloc_Sloth = round(float(qty_by_strategy['sloth'] / total_qty), 2)
        result.alloc_Primero = round(float(qty_by_strategy['primero'] / total_qty), 2)
    ...
    def consolidate_strategies(self, toggle):
        self.output['send_to_roster'] = True
        self.output['send_to_sheet'] = True
        self.output['full_size'] = 'n/a'
        self.output['combo_schema'] = False
        self.output['alloc_Jump'] = -1.0
        self.output['alloc_Primero'] = -1.0
        self.output['alloc_Sloth'] = -1.0
        if toggle == '1':
            ...
            columns_to_overwrite = ['NAV_diff', 'pct_of_20d_ATV', 'rank_long', 'rank_short', 'rank_avg', 'rank_vs_15d_avg', 'actual_shares', 'target_shares']
    ...
    def final_cleanup(self):
        ...
        self.output = self.output.loc[:,['trading_platform', 'strategy', 'book', 'ticker', 'actual_shares', 'target_shares', 'NAV_diff', 'first_leg', 'amount_1st_leg', 'amount_1st_leg_LL_on', 'price', 'rank_long', 'rank_avg', 'rank_short', 'todays_price_return', 'next_report_date', 'cfo_growth', 'low_liquidity', 'limit_price', 'pct_of_20d_ATV', 'broker_ticker', '20d_ATV', 'direction_human', 'rank_vs_15d_avg', 'reversal', 'combo_schema', 'alloc_Primero', 'alloc_Jump', 'alloc_Sloth', 'cons_type', 'cons_id', 'send_to_roster', 'send_to_sheet']]
        self.output.rename(columns={'first_leg' : 'direction', 'amount_1st_leg' : 'quantity', 'amount_1st_leg_LL_on' : 'qty_LL_on'}, inplace=True)

    def execute(self, consolidate):
        ...
        self.consolidate_strategies(consolidate)
